...
- (NSString *) processAcclerometer:(NSMutableArray *) acceleromterData
{
    ...
    double oneNorm_X, oneNorm_Y, oneNorm_Z;
    double infinityNorm, fNorm; 
    for(int i = 0; i < size; i++)
    {
        ...
        if(acceleration.y < min_X) min_Y = acceleration.y;
        if(acceleration.z < min_X) min_Z = acceleration.z;
    ...
    double skewnessSum_X, skewnessSum_Y, skewnessSum_Z;
    double kurtosisSum_X, kurtosisSum_Y, kurtosisSum_Z;
    double squaredSum_X_Bottom, squaredSum_Y_Bottom, squaredSum_Z_Bottom;
...
- (NSString *) processGryo:(NSMutableArray *) gryoData
{
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        if(gryo.y < min_X) min_Y = gryo.y;
        if(gryo.z < min_X) min_Z = gryo.z;
